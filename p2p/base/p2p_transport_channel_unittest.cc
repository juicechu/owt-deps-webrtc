 ...
 protected:
  ...
  std::unique_ptr<P2PTransportChannel> CreateChannel(
    ...
    auto channel = P2PTransportChannel::Create(
        "test content name", media_type, component, GetAllocator(endpoint),
...
TEST_F(P2PTransportChannelPingTest, TestMaxOutstandingPingsFieldTrial) {
  ...
  FakePortAllocator pa(rtc::Thread::Current(), nullptr);
  P2PTransportChannel ch("max",  cricket::MEDIA_TYPE_VIDEO, 1, &pa);
  ch.SetIceConfig(ch.config());
  PrepareChannel(&ch);
  ch.MaybeStartGathering();
  ch.AddRemoteCandidate(CreateUdpCandidate(LOCAL_PORT_TYPE, "1.1.1.1", 1, 1));
...
class P2PTransportChannelMostLikelyToWorkFirstTest
 ...
 public:
  ...
  P2PTransportChannel& StartTransportChannel(
    ...
    channel_.reset(new P2PTransportChannel("checks",  cricket::MEDIA_TYPE_VIDEO, 1, allocator()));
...
TEST(P2PTransportChannelResolverTest, HostnameCandidateIsResolved) {
  ...
  auto channel =
      P2PTransportChannel::Create("tn", cricket::MEDIA_TYPE_VIDEO, 0, &allocator, &resolver_fixture);
...
TEST_F(P2PTransportChannelPingTest, TestForgetLearnedState) {
  ...
  FakePortAllocator pa(rtc::Thread::Current(), nullptr);
  auto ch = P2PTransportChannel::Create("ping sufficiently", cricket::MEDIA_TYPE_VIDEO, 1, &pa, nullptr,
                                        nullptr, &factory);
